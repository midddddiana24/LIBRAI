...
def receipt(x,db):
    limit=max_renewals(db);due=x.due_at if x.due_at.tzinfo else x.due_at.replace(tzinfo=timezone.utc);return {"id":x.id,"transaction_id":f"BOR-{x.id:08d}","user_id":x.user_id,"user_name":x.user.display_name,"student_id":x.user.student_id,"course":x.user.course,"book_copy_id":x.book_copy_id,"accession_number":x.book_copy.accession_number,"book_title":x.book_copy.book.title,"borrowed_at":x.borrowed_at,"due_at":x.due_at,"returned_at":x.returned_at,"status":x.status.lower(),"renewal_count":x.renewal_count,"max_renewals":limit,"can_renew":x.status==BorrowingStatus.ACTIVE and due>=datetime.now(timezone.utc) and x.renewal_count<limit}
...
def list_(user_id:int|None=None,verification_token:str|None=None,status:str|None=None,offset:int=Query(0,ge=0),limit:int=Query(50,ge=1,le=100),admin:Admin|None=Depends(get_optional_admin),db:Session=Depends(get_db)):
    ...
    stmt=select(Borrowing).options(joinedload(Borrowing.user),joinedload(Borrowing.book_copy).joinedload(BookCopy.book))
...
def get_(borrowing_id:int,_admin:Admin=Depends(get_current_admin),db:Session=Depends(get_db)):
    x=db.scalar(select(Borrowing).options(joinedload(Borrowing.user),joinedload(Borrowing.book_copy).joinedload(BookCopy.book)).where(Borrowing.id==borrowing_id))
