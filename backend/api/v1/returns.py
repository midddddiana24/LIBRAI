...
def list_(offset:int=Query(0,ge=0),limit:int=Query(50,ge=1,le=100),_admin:Admin=Depends(get_current_admin),db:Session=Depends(get_db)):
    stmt=select(Borrowing).options(joinedload(Borrowing.user),joinedload(Borrowing.book_copy).joinedload(BookCopy.book)).where(Borrowing.status==BorrowingStatus.RETURNED);total=db.scalar(select(func.count()).select_from(stmt.subquery())) or 0;return {"items":[{"id":x.id,"transaction_id":f"RET-{x.id:08d}","user_id":x.user_id,"user_name":x.user.display_name,"student_id":x.user.student_id,"course":x.user.course,"book":x.book_copy.book.title,"accession_number":x.book_copy.accession_number,"returned_at":x.returned_at,"status":"returned"} for x in db.scalars(stmt.offset(offset).limit(limit)).unique()],"total":total}
