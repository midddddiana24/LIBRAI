...
from backend.models.entities import Book, BookCopy, Borrowing, BorrowingStatus, Category, Fine, FineStatus, User
...
def report_data(db: Session, report_type: str, start_date=None, end_date=None) -> tuple[list[str],list[list]]:
    ...
    elif report_type=="unpaid_fines":
        headers=["Fine","Student","Name","Book","Amount","Assessed"]
        rows=[[f.id,f.user.student_id,f.user.display_name,f.borrowing.book_copy.book.title if f.borrowing else "",f"{f.amount_cents / 100:.2f}",f.assessed_at.isoformat()] for f in db.scalars(select(Fine).where(Fine.status==FineStatus.UNPAID)).all()]
